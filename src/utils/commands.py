...
class Orchestrator:
    ...
    def run_orchestartion(orc):
        ...
        list_modules_name=[module.__name__ for module in orchestrator_file.list_modules]
        logger.debug(f"The list of modules that will be executed are {list_modules_name}")
        
        logger.info("Starting all modules execution...")
        for module in orchestrator_file.list_modules:
            logger.info("Queueing modules execution for module [{module}]...")
            module().run()
