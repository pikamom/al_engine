...
class Test(Module):
    ...
    def run(self):
        ...
        today = datetime.now()
        logger.debug(f"Today's date is {today}, Webscrapper will start looking for data from yesterday in a backward fashion")
        ...
        while back_tracked_days < number_of_backtrack_days:
            ...
            logger.debug(f"Starting the webscraping process from [{date_format_for_df}]")
            ...
            if response.status_code == 200:
                logger.debug(f"Successful in obtaining metal future prices for {date_format_for_df}")
                ...
                logger.debug(f"There are no futures prices data for {date_format_for_df} or connection was not successful")
